...
if not locations:
    ...
    if st.sidebar.button('🚀 產生熱力圖', key='pages_4_hm_button'):
        ...
        # 防呆檢查
        if TARGET_COLUMN_NAME not in results_df.columns:
            st.error(f"程式中止：指定的目標欄位 '{TARGET_COLUMN_NAME}' 在資料中找不到！")
            st.warning("請根據下方列表，找出正確的欄位名稱，並修改程式碼第 14 行的 `TARGET_COLUMN_NAME` 變數。")
            st.info(f"目前資料中所有可用的欄位：\n{results_df.columns.tolist()}")
            st.stop()
        ...
        if not display_df.empty:
            ...
            if heatmap_data is not None and not heatmap_data.empty:
                ...
                heatmap_data_reindexed = heatmap_data.reindex(locations).dropna(how='all')
