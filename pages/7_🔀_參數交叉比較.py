...
station = st.sidebar.selectbox("① 選擇測站", locations, key='pages_7_xc_station', on_change=reset_analysis_state)
...
if not station_specific_years:
    st.sidebar.error(f"測站 '{station}' 找不到任何年份資料。")
    st.error(f"❌ 找不到測站 **{station}** 的任何年份資料，請嘗試選擇其他測站。")
    st.stop()
...
with st.spinner(f"正在載入 {station} 在 {year}年 的資料..."):
    df_year = load_year_data(base_data_path, station, year)

if df_year is None or df_year.empty:
    st.error(f"❌ 找不到 {station} 在 {year}年 的任何資料。")
    st.session_state.analysis_run = False
else:
    ...
    with st.expander(f"📊 資料儀表板：點此查看 {station} 在 {year} 年的數據概覽", expanded=True):
    ...
    if df_analysis.empty or len(df_analysis) < 2:
        ...
        st.markdown(f"### 交叉分析結果：{station} ({year}年)")
