...
# 座標自我修正與補全
known_buoy_coords = {
    '南灣資料浮標': {'lat': 21.94527, 'lon': 120.76},
    '小琉球資料浮標': {'lat': 22.3131, 'lon': 120.3639},
    '彌陀資料浮標': {'lat': 22.7653, 'lon': 120.165},
    '東吉嶼資料浮標': {'lat': 23.23666, 'lon': 119.64861},
    '潮境資料浮標': {'lat': 25.14416, 'lon': 121.80833},
    '臺東資料浮標': {'lat': 22.7242, 'lon': 121.1442},
    '蘇澳資料浮標': {'lat': 24.6256, 'lon': 121.8761},
    '鵝鑾鼻資料浮標': {'lat': 21.900278, 'lon': 120.8225},
    '龍洞資料浮標': {'lat': 25.0969, 'lon': 121.9222}
}
for station_name, coords in known_buoy_coords.items():
    if station_name not in station_coords or not isinstance(station_coords.get(station_name), dict) or \
       station_coords[station_name].get('lat') is None or station_coords[station_name].get('lon') is None:
...
if analysis_mode == "靜態地圖":
    ...
    if map_locations:
        df_map = pd.DataFrame.from_dict(map_locations, orient='index')
        # 靜態地圖也使用台灣中心視角
